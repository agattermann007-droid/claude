"""Build 6.70: wie bust_ana.py mit eng11 und x70.VAR.
Build 6.50: Busts auf den Fremddaten je Variante: Datum (Monat), Art (1 Boden, 2 Floating, 3 Tagesverlust), Anzahl Konten.
Aufruf (in Replikat_v6): python a70_bust.py "Variante|Variante" [seeds] [step]"""
import sys, os, numpy as np, collections
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import eng11 as E, evl6 as V, r6, x48, x60, x70
...
for name in names:
    kw, gpx, frisk, rule, per, extra, extra_gp = x60.unpack(x70.VAR[name])
    blks, info, mk = x60.setup("ext", rule)
    V._MK = mk
    mk.r21["reg"] = x48.r21_regime("ext", mk)
