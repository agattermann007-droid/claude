"""DEADBAND Replikat v11 (Build 6.70) - eng10 plus:
...
gueltig macht (Kopfraum bis zur Schwelle inkl. sv_margin), hoechstens bis sv_cap % vom Startsaldo x Pufferkurve und nur innerhalb
der Budgets; nur solange dem Zyklus noch gueltige Tage fehlen und der Tag noch nicht gueltig ist. sv_on 1 = nur, wenn die
Schwelle mit sv_cap erreichbar ist, 2 = immer bis sv_cap. Noise auch short (nz_short, Risiko nz_s_risk): Short unter der unteren
Bandgrenze LB = min(Eroeffnung, Vortagesschluss) x (1 - k sigma), Ausstieg ueber LB, Stops oberhalb; Hedging-Sperre gegen alle
NAS-Longs. Trade-Protokoll Spalte 8 = Groessenfaktor beim Einstieg, 9 = Richtung.
Mit den Voreinstellungen rechnet eng10 wie eng9 (t_eng10.py).

DEADBAND Replikat v9 (Build 6.50, Netto) - eng8 plus: Schutz gueltiger Tage je Modul (vp_mods, GP vpx), weitere
Schutzformen (vp_on 5: enger Stop fuer Einstiege nach gueltigem Tag, 6: Einstieg nur mit Kopfraum >= vp_k x Risiko),
Trade-Protokoll mit Einstiegs-Zustand (Spalte 4: 1 = Tag schon gueltig und Zyklus braucht Tage, 2 = gueltig, Tage komplett;
Spalte 5: Kopfraum realisiert - Schwelle beim Einstieg). Mit den Voreinstellungen rechnet eng9 wie eng8 (t_eng9.py).

DEADBAND Replikat v8 (Build 6.40, Auszahlungstakt) - eng7 plus Schutz gueltiger Tage (vp_on: neue Einstiege nach einem
gueltigen Tag nur mit so viel Stop-Risiko, dass der Tag gueltig bleibt), Abschluss-Ernte ganz oder mit Rest auf Einstand
(bank_full, bank_be) und Tages-Diagnose (Verteilung des realisierten Tagesergebnisses, verlorene gueltige Tage, Wartetage).
Mit den Voreinstellungen rechnet eng8 wie eng7 (t_eng8.py).
...
NG = 24                 # generische Stroeme, je Strom ein Platz je Symbol (6.60: 24 statt 14)
NZ0 = 6
NZ1 = 9
G0 = 9
NSLOT = G0 + 2 * NG
...
GPN = ["on", "risk", "tp1r", "tp1f", "be", "maxtrades", "maxloss", "budget", "ripeclose",
       "trail_from", "trail_dist", "harv", "usefak", "maxopen_sym", "cool_excl", "tpdelay", "vpx"]
...
PN = [
    ...
    # --- 6.30: Noise-Ausstiege (Voreinstellung = wie eng6)
    "nz_tp1r", "nz_tp1f", "nz_be", "nz_tp",
    # --- 6.40: Auszahlungstakt (Voreinstellung = wie eng7)
    "vp_on", "vp_margin", "vp_from", "bank_full", "bank_be",
    # --- 6.50: Netto (Voreinstellung = wie eng8)
    "vp_mods", "vp_k", "vp_minfrac", "vp_r21_to", "vp_r21_dd", "vp_r21_reg",
    # --- 6.60: Groesse fuer den gueltigen Tag (Voreinstellung = wie eng9)
...
# ------------------------------------------------------------------ Markt + Signale
class Market:
    def __init__(self, sigfile="sig5.pkl", D=None, S=None):
        if D is None:
            with open(os.path.join(P.OUT, sigfile), "rb") as f:
                S = pickle.load(f)
            D = P.build()
        # fremde Daten ohne S: keine Signale der alten Module (nur generische Stroeme)
        self.D = D
        syms = ("XAU", "NAS")
    ...
    def _empty_old(self, D, allt):
        n_ev = len(allt)
        z64 = np.zeros(0, np.int64); zf = np.zeros(0)
        self.db = dict(ev=z64, sym=z64, dir=z64, rd=zf, f=zf, m15=z64, nyh=zf, i5=z64)
        self.r21 = dict(ev=z64, sym=z64, tf=z64, dir=z64, rd=zf, i5=z64, first=z64)
        self.nz = dict(ev=z64, em=z64, close=zf, UB=zf, vw=zf, dist=np.zeros((0, 3)), entry_ok=z64, eod_ev=z64, day=z64)
        for d_ in (self.db, self.r21, self.nz):
            d_["next"] = np.zeros(n_ev + 1, np.int64)
        self.nz_eod = z64
        self.ev_nzeod = np.zeros(n_ev + 1, np.int64)

    def set_generic(self, sigs):
        """sigs: Liste von dicts je Strom-Signalblock mit Schluesseln str, sym, dir, t_entry (NY-Minute des Einstiegs =
        Open der M5-Kerze), rd (Stop-Abstand in Preis), tp (Ziel in R, 0 = keins), t_exit (NY-Minute: Ausstieg am Open der
        ersten Kerze ab diesem Zeitpunkt), w (Risikogewicht)."""
        n_ev = len(self.ev_t)
        cols = {k: [] for k in ("ev", "str", "sym", "dir", "rd", "tp", "xev", "w")}
        for blk in sigs:
            sy = np.asarray(blk["sym"], np.int64)
            te = np.asarray(blk["t_entry"], np.int64); tx = np.asarray(blk["t_exit"], np.int64)
            ev = np.searchsorted(self.ev_t, te, side="left"); xev = np.searchsorted(self.ev_t, tx, side="left")
            ok = (ev < n_ev) & (xev > ev)
            ev_c = np.minimum(ev, n_ev - 1)
            ok &= self.ev_i[sy, ev_c] >= 0
            ok &= (self.ev_t[ev_c] - te) <= 10
            m = int(ok.sum())
            cols["ev"].append(ev[ok]); cols["xev"].append(np.minimum(xev[ok], n_ev))
            cols["str"].append(np.full(m, int(blk["str"]), np.int64)); cols["sym"].append(sy[ok])
            cols["dir"].append(np.asarray(blk["dir"], np.int64)[ok]); cols["rd"].append(np.asarray(blk["rd"], float)[ok])
            cols["tp"].append(np.asarray(blk["tp"], float)[ok] if np.ndim(blk["tp"]) else np.full(m, float(blk["tp"])))
            cols["w"].append(np.asarray(blk["w"], float)[ok] if np.ndim(blk["w"]) else np.full(m, float(blk["w"])))
        if cols["ev"]:
            G = {k: np.concatenate(v) for k, v in cols.items()}
        else:
            G = dict(ev=np.zeros(0, np.int64), str=np.zeros(0, np.int64), sym=np.zeros(0, np.int64), dir=np.zeros(0, np.int64),
                     rd=np.zeros(0), tp=np.zeros(0), xev=np.zeros(0, np.int64), w=np.zeros(0))
        order = np.lexsort((G["sym"], G["str"], G["ev"]))
        G = {k: np.ascontiguousarray(v[order]) for k, v in G.items()}
        for k in ("ev", "str", "sym", "dir", "xev"):
            G[k] = G[k].astype(np.int64)
        G["next"] = np.searchsorted(G["ev"], np.arange(n_ev + 1), side="left").astype(np.int64)
        self.g = G
...
def run_path(Pv, d0, d1, seed,
    ...
    nz_tp1r = Pv[132]; nz_tp1f = Pv[133]; nz_be = Pv[134]; nz_tp = Pv[135]
    vp_on = int(Pv[136]); vp_margin = Pv[137]; vp_from = Pv[138]; bank_full = Pv[139] > 0.5; bank_be = Pv[140] > 0.5
    ...
    p_v1 = np.zeros(NSLOT)
    p_t1l = np.zeros(NSLOT)            # 6.30: T1-Niveau in R je Position (generisch)
    p_xev = np.full(NSLOT, -1, np.int64)      # generisch: erzwungener Ausstieg (Ereignisindex, Kerzen-Open)
    p_pv = np.zeros(NSLOT); p_hd = np.zeros(NSLOT)   # 6.50: Einstieg nach gueltigem Tag (1/2), Kopfraum beim Einstieg
    p_r0 = np.zeros(NSLOT)                           # 6.50: Stop-Risiko beim Einstieg ($, Protokoll)
    ...
    g_trades = np.zeros(2 * NG, np.int64)
    ...
    day_locked = False; day_wv = False
    min_buf = floor_dist          # 6.40: kleinster Abstand Equity (schlechtester Kurs) - Boden
    # Serien
    ...
    while p < end_ev:
        ...
        if di != dayidx:
            ...
            for k in range(NSLOT):
                if p_on[k]:
                    ...
                    p_swap[k] += sw
            # 6.40: Tages-Diagnose (realisiertes Tagesergebnis, verlorene gueltige Tage, Wartetage)
            if day_had:
                if day_real < 0.0:
                    st[56] += 1
                elif day_real < 25.0:
                    st[57] += 1
                elif day_real < needday:
                    st[58] += 1
                else:
                    st[59] += 1
                if day_wv and day_real < needday:
                    st[60] += 1
            elif mode == 0:
                st[62] += 1
            if mode != 0:
        ...
        if we_on and we_reentry and (dow == 0 and nyh >= 18.0 or dow == 1):
            for k in range(NSLOT):
                ...
                p_pv[k] = 0.0; p_hd[k] = 0.0; p_r0[k] = 0.0; p_fk[k] = -1.0
        ...
        for k in range(NZ0, NZ0 + nz_parts):
            if p_on[k]:
                ...
                nz_d = p_dir[k]
        eodnow = ev_nzeod[p] == 1
        nz_exit = False                     # 6.60: Long raus unter UB, Short raus ueber LB
        if zcur >= 0:
            if nz_d > 0 and z_close[zcur] < z_UB[zcur]:
                nz_exit = True
            if nz_d < 0 and z_close[zcur] > z_LB[zcur]:
                nz_exit = True
        if nz_open > 0 and (eodnow or nz_exit or (we_now)):
            ii = ev_i[1, p]
            if ii >= 0:
                for k in range(NZ0, NZ0 + nz_parts):
                    if not p_on[k]:
                        continue
                    px = o5[1, ii] + (sp5[1, ii] if p_dir[k] < 0 else 0.0)
                    pnl = (px - p_entry[k]) * p_dir[k] * p_lots[k] * 10.0 + p_swap[k]
        ...
        if day_entry_stop > 0.0 and (eq_now - (day_start_bal + day_ref_plus)) <= -start * day_entry_stop / 100.0:
            entries_ok = False
        # 6.40: Schutz gueltiger Tage - nach einem gueltigen Tag neue Einstiege nur mit so viel Stop-Risiko, dass der Tag
        # auch dann gueltig bleibt, wenn alle offenen Positionen und der neue Trade am Stop schliessen (vp_on 1), bzw. gar keine (2)
        # vp_on 3/4 wie 2/1, aber nur solange dem Zyklus (ohne heute) noch gueltige Tage fehlen - danach zaehlt nur der Gewinn
        vp_act = False
        r21_dd = False          # 6.50: RSI21 nach gueltigem Tag nur ohne Rueckgang (vp_r21_dd)
        if vp_r21_dd > 0:
            if (vp_r21_dd & 1) and _ddf(eq_now, peak, floor_dist, start, ddfull, ddmin, ddfmin) < 1.0:
                r21_dd = True
            if (vp_r21_dd & 2) and eq_now < start:
                r21_dd = True
        vp_blk = False          # 6.50: Sperre nur fuer die Module in vp_mods (vp 2/3 mit vp_mods < 15)
        vp_tight = False        # 6.50: vp 5 = enger Stop, 6 = Kopfraum >= vp_k x Risiko
        pv_now = 0.0
        if day_had and day_real >= needday:
            pv_now = 1.0 if valid_days < needvalid else 2.0
        hd_now = day_real - needday
        if vp_on > 0 and entries_ok and day_had and day_real >= needday and nyh >= vp_from \
                and (vp_on <= 2 or valid_days < needvalid):
            if vp_on == 2 or vp_on == 3:
                if vp_mods == 15:
                    entries_ok = False
                else:
                    vp_blk = True
            elif vp_on == 5 or vp_on == 6 or vp_on == 7:
                vp_tight = True
            else:
                vp_act = True
        ...
        while dbi < ndb and db_ev[dbi] == p:
            ...
            p_pv[k] = pv_now; p_hd[k] = hd_now; p_r0[k] = (p_entry[k] - p_sl[k]) * p_dir[k] * p_lots[k] * mpp[p_sym[k]]; p_fk[k] = fak
        ...
        while ri < nr and r_ev[ri] == p:
            ...
            if vp_act:
                ...
                if rest4 < rest:
                    rest = rest4
                    idea_lim = True
            if vp_tight and vp_on == 6 and day_real - needday - vp_margin - orisk_all < vp_k * r:
                st[64] += 1
                continue
            if vp_tight and vp_on == 7:
                r *= vp_k
            if rest < r:
                r = rest
            if r <= 0.0:
            ...
            if slip_frac > 0.0:
                ent += d * np.random.random() * slip_frac * spr
            sl_d = rd
            if vp_tight and vp_on == 5:
                head = day_real - needday - vp_margin - orisk_all
                mx_d = head / (lots * mpp[sy]) if head > 0.0 else 0.0
                if mx_d < rd:
                    st[63] += 1
                    if mx_d < vp_minfrac * rd or mx_d < 6.0 * spr or mx_d <= 0.0:
                        st[64] += 1
                        continue
                    sl_d = mx_d
            k = ke
            rrr = r21_rr[sy]
            p_on[k] = True; p_sym[k] = sy; p_dir[k] = d; p_entry[k] = ent; p_sl[k] = ent - d * sl_d; p_tp[k] = ent + d * rrr * rd
        ...
        nzd = 0
        if zcur >= 0:
            if z_close[zcur] > z_UB[zcur]:
                nzd = 1
            elif nz_short and z_close[zcur] < z_LB[zcur]:
                nzd = -1                    # 6.60: Noise short
        ...
        if zcur >= 0 and nz_on and entries_ok and not (vp_blk and (vp_mods & 4)) and z_entry[zcur] == 1 and nzd != 0 and not skipmask_z[zcur]:
            hed = False
            for q in range(NSLOT):
                if p_on[q] and p_sym[q] == 1 and p_dir[q] != nzd:
                    hed = True
                if w_on[q] and (q == 1 or q == 3 or q == 5) and w_dir[q] != nzd:
                    hed = True
            ii = ev_i[1, p]
            if (not hed) and ii >= 0:
                for q in range(nz_parts):
                    ...
                    if idea_cap > 0.0:
                        rest3 = start * idea_cap / 100.0 - _idea_risk(p_on, p_sym, p_dir, p_entry, p_sl, p_lots, mpp, 1, nzd)
                        if rest3 < rest:
                            rest = rest3
                    ...
                    if lots * dist * 10.0 > rest + 1e-9:
                        ...
                        if lots < 0.01 - 1e-12:
                            continue
                    lots = np.round(lots, 2)
                    if not _margin_ok(p_on, p_sym, p_dir, p_lots, p_last, mpp, lev, idea_margin, 1, nzd, lots, o5[1, ii], eq_now):
                        continue
                    spr = sp5[1, ii]
                    ent = o5[1, ii] + (spr if nzd > 0 else 0.0)
                    if slip_frac > 0.0:
                        ent += nzd * np.random.random() * slip_frac * spr
                    sl_d = dist
                    if vp_tight and vp_on == 5:
                        head = day_real - needday - vp_margin - orisk_all
                        mx_d = head / (lots * 10.0) if head > 0.0 else 0.0
                        if mx_d < dist:
                            st[63] += 1
                            if mx_d < vp_minfrac * dist or mx_d < 6.0 * spr or mx_d <= 0.0:
                                st[64] += 1
                                continue
                            sl_d = mx_d
                    p_on[k] = True; p_sym[k] = 1; p_dir[k] = nzd; p_entry[k] = ent; p_sl[k] = ent - nzd * sl_d
                    p_tp[k] = ent + nzd * nz_tp * dist if nz_tp > 0.0 else 0.0
                    p_rd[k] = dist; p_ref[k] = ent; p_lots[k] = lots; p_swap[k] = 0.0; p_mfe[k] = 0.0; p_be[k] = True
                    p_t1[k] = nz_tp1r <= 0.0; p_i5[k] = ii; p_comm[k] = 0.0; p_acc[k] = 0.0; p_last[k] = o5[1, ii]; p_v1[k] = 0.0
                    if nz_tp1r > 0.0 and nz_tp1f > 0.0:
                        v1n = np.floor(lots * nz_tp1f / 0.01 + 1e-9) * 0.01
                        if v1n >= 0.01 and lots - v1n >= 0.01:
                            p_v1[k] = np.round(v1n, 2)
        ...
        # ---- Generische Signalstroeme (neue Strategien)
        while gi < ngs and g_ev[gi] < p:
            gi += 1
        while gi < ngs and g_ev[gi] == p:
            a = gi
            gi += 1
            s_ = g_str[a]
            if GP[s_, 0] <= 0.5 or not entries_ok:
                continue
            ...
            for q in range(NSLOT):
                if p_on[q] and p_sym[q] == sy and p_dir[q] != d:
                    hed = True
                if w_on[q] and q < NZ0 and ((q == sy) or (q == 2 + sy) or (q == 4 + sy)) and w_dir[q] != d:
                    hed = True
            if hed:
                continue
            rd = g_rd[a]
            ...
            for q in range(NSLOT):
                if p_on[q]:
                    ...
                    if dist > 0.0:
                        rr_ = dist * p_lots[q] * mpp[p_sym[q]]
                        orisk_all += rr_
                        if q >= G0 and (q - G0) // 2 == s_:
                            orisk_g += rr_
            rest = start * gesamtbudget / 100.0 - orisk_all
            if GP[s_, 7] > 0.0:
                rest_g = start * GP[s_, 7] / 100.0 - orisk_g
                if rest_g < rest:
                    rest = rest_g
            if idea_cap > 0.0:
                rest3 = start * idea_cap / 100.0 - _idea_risk(p_on, p_sym, p_dir, p_entry, p_sl, p_lots, mpp, sy, d)
            ...
            if vp_act:
                ...
                if rest4 < rest:
                    rest = rest4
            if vp_tight and GP[s_, 16] <= 0.5 and vp_on == 6 and day_real - needday - vp_margin - orisk_all < vp_k * r:
                st[64] += 1
                continue
            if vp_tight and GP[s_, 16] <= 0.5 and vp_on == 7:
                r *= vp_k
            # 6.60: Groesse fuer den gueltigen Tag - der Ziel-Treffer soll den Tag allein gueltig machen
            tpr_sv = g_tp[a]
            if sv_on > 0 and tpr_sv > 0.0 and mode == 0 and valid_days < needvalid and not (day_had and day_real >= needday):
                need_sv = needday * (1.0 + sv_margin) - day_real
                win_lot = tpr_sv * rd * mpp[sy] - comm[sy]
                if need_sv > 0.0 and win_lot > 0.0:
                    lots_sv = np.ceil(need_sv / win_lot / 0.01 - 1e-9) * 0.01
                    r_sv = lots_sv * rd * mpp[sy]
                    cap_sv = start * sv_cap / 100.0 * (fak if GP[s_, 12] > 0.5 else 1.0) * g_w[a]
                    if r_sv > r and (sv_on == 2 or r_sv <= cap_sv + 1e-9):
                        r = r_sv if r_sv <= cap_sv else cap_sv
                        st[67] += 1
            if rest < r:
                r = rest
            if r <= 0.0:
            ...
            if slip_frac > 0.0:
                ent += d * np.random.random() * slip_frac * spr
            sl_d = rd
            if vp_tight and GP[s_, 16] <= 0.5 and vp_on == 5:
                head = day_real - needday - vp_margin - orisk_all
                mx_d = head / (lots * mpp[sy]) if head > 0.0 else 0.0
                if mx_d < rd:
                    st[63] += 1
                    ...
                    sl_d = mx_d
            tpr = g_tp[a]
            p_on[k] = True; p_sym[k] = sy; p_dir[k] = d; p_entry[k] = ent; p_sl[k] = ent - d * sl_d
            p_tp[k] = ent + d * tpr * rd if tpr > 0.0 else 0.0
            p_rd[k] = rd; p_ref[k] = ent; p_lots[k] = lots; p_swap[k] = 0.0; p_mfe[k] = 0.0
            t1l = GP[s_, 2] if GP[s_, 2] > 0.0 else ((-GP[s_, 2] * tpr) if (GP[s_, 2] < 0.0 and tpr > 0.0) else 0.0)
            p_t1l[k] = t1l
            p_be[k] = t1l <= 0.0; p_t1[k] = t1l <= 0.0
            p_i5[k] = ii; p_m15[k] = 0; p_tmin[k] = days[dayidx] * 1440 + nymin
            p_pv[k] = pv_now; p_hd[k] = hd_now; p_r0[k] = (p_entry[k] - p_sl[k]) * p_dir[k] * p_lots[k] * mpp[p_sym[k]]; p_fk[k] = fak
            p_comm[k] = comm[sy] * lots; bal -= p_comm[k]; p_acc[k] = 0.0; p_last[k] = op; p_v1[k] = 0.0
            p_xev[k] = g_xev[a]
            p_ext[k] = False
            if t1l > 0.0 and GP[s_, 3] > 0.0:
                v1g = np.floor(lots * GP[s_, 3] / 0.01 + 1e-9) * 0.01
                if v1g >= 0.01 and lots - v1g >= 0.01:
                    p_v1[k] = np.round(v1g, 2)
            g_trades[2 * s_ + sy] += 1
            day_trades_all += 1
            st[25] += 1
            if cyc_start < 0:
        ...
        if anyon:
            ...
            if bal + worst - flo < min_buf:
                min_buf = bal + worst - flo
                st[66] = float(days[dayidx])
            if bal + worst <= flo:
                bust = 1
        ...
        for k in range(NSLOT):
            ...
            px = 0.0
            closed = False
            if k >= G0 and p_xev[k] >= 0 and p >= p_xev[k]:
                px = o5[sy, ii] + (sp if d < 0 else 0.0)                  # erzwungener Ausstieg am Kerzen-Open
                closed = True
            elif hit_sl:
                px = p_sl[k]
                ...
                s_ = (k - G0) // 2
                if p_t1l[k] > 0.0 and not p_t1[k] and ii - p_i5[k] >= int(GP[s_, 15]):
                    lvl = p_ref[k] + d * p_t1l[k] * p_rd[k]
                    ...
                    if t1hit:
                        ...
                        if p_v1[k] > 0.0:
                            ...
                            p_v1[k] = 0.0
                        if GP[s_, 4] > -9.0:
                            cand = p_ref[k] + d * GP[s_, 4] * p_rd[k]
                            if (cand - p_sl[k]) * d > 0.0:
                                p_sl[k] = cand
                        p_be[k] = True
                if GP[s_, 9] > 0.0 and p_mfe[k] >= GP[s_, 9]:
                    cand = p_ref[k] + d * (p_mfe[k] - GP[s_, 10]) * p_rd[k]
                    if (cand - p_sl[k]) * d > 0.0:
                        p_sl[k] = cand
            else:
                if k < 2:
                    ...
                    if (r21_exitbars > 0 and held5 >= r21_exitbars) or (r21_exitdays > 0 and (tnow - p_tmin[k]) >= r21_exitdays * 1440):
                        ...
                        if r21_tp1r > 0.0 and not p_t1[k] and held5 >= 1:
                        ...
                        if r21_trail_from > 0.0 and p_mfe[k] >= r21_trail_from:
                            ...
                            if (cand - p_sl[k]) * d > 0.0:
                                p_sl[k] = cand
                elif nz_tp1r > 0.0 and not p_t1[k] and ii - p_i5[k] >= 1:
                    # 6.30: Noise-Teil: Teilgewinn und Stop auf Einstand ab nz_tp1r R (R = Stop-Abstand des Teils)
                    lvl = p_ref[k] + d * nz_tp1r * p_rd[k]
                    t1hit = (hi >= lvl) if d > 0 else (lo + sp <= lvl)
                    if t1hit:
                        p_t1[k] = True
                        if p_v1[k] > 0.0:
        ...
        if bank_ok and not day_locked and day_real < needday and nyh >= bank_from and nyh < bank_to and mode <= 1:
            ...
            for k in range(NSLOT):
                ...
                if k >= 2 and k < NZ0 and (bank_mods & 2) == 0:
                    continue
                if k >= NZ0 and k < NZ1 and (bank_mods & 4) == 0:
                    continue
                if k >= G0 and ((bank_mods & 8) == 0 or GP[(k - G0) // 2, 11] <= 0.5):
                    continue
                sy = p_sym[k]
                ii = ev_i[sy, p]
                if ii < 0:
            ...
            if day_real + gain >= need:
                for k in range(NSLOT):
                    ...
                    if lots < 0.01:
                        lots = 0.01
                    if lots > p_lots[k] - 0.01 or bank_full:
                        lots = p_lots[k]
                    lots = np.round(lots, 2)
                    st[36] += 1
                    if lots >= p_lots[k]:
                        ...
                        p_lots[k] = np.round(p_lots[k] - lots, 2)
                        if bank_be:
                            # 6.40: Rest auf Einstand, damit er den eben gueltig gemachten Tag nicht wieder kippt
                            if (p_entry[k] - p_sl[k]) * p_dir[k] > 0.0:
                                p_sl[k] = p_entry[k]
                    if day_real >= need:
                        break
            ...
            if mode == 1:
                ...
                for k in range(2, NSLOT):
                    ...
                    if k < NZ0 and not r21_ripeclose:
                        continue
                    if k >= NZ0 and k < NZ1 and not nz_ripeclose:
                        continue
                    if k >= G0 and GP[(k - G0) // 2, 8] <= 0.5:
                        continue
                    sy = p_sym[k]
                    ii = ev_i[sy, p]
                    if ii < 0:
        ...
        if not anyon:
            ...
            if mode == 0:
                ...
                if zi < nzc and z_ev[zi] < nxt and z_ev[zi] > p:
                    nxt = z_ev[zi]
                if gi < ngs and g_ev[gi] < nxt and g_ev[gi] > p:
                    nxt = g_ev[gi]
            if dayidx + 1 < nd:
...
def make_masks(mk, seed, frac):
    ...
    if frac <= 0.0:
        return (np.zeros(len(mk.db["ev"]), np.bool_), np.zeros(len(mk.r21["ev"]), np.bool_),
                np.zeros(len(mk.nz["ev"]), np.bool_), np.zeros(ng_, np.bool_))
    return (rng.random(len(mk.db["ev"])) < frac, rng.random(len(mk.r21["ev"])) < frac, rng.random(len(mk.nz["ev"])) < frac,
            rng.random(ng_) < frac)


def gparams(streams):
    """streams: Liste von dicts (je Strom Parameter nach GPN). Fehlende Stroeme = aus."""
    GP = np.zeros((NG, len(GPN)))
    for s_ in range(NG):
        GP[s_, GPI["be"]] = -99.0
        GP[s_, GPI["usefak"]] = 1.0
        GP[s_, GPI["ripeclose"]] = 1.0
        GP[s_, GPI["tpdelay"]] = 1.0
    for s_, kw in enumerate(streams):
        for n, v in kw.items():
            GP[s_, GPI[n]] = float(v)
    return GP
