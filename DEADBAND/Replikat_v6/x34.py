"""6.00 Sicher/Ertrag mit Auszahlung ab 4 % (400 $) statt 3 %: GFT 2022-26 (16 Stoerungen) und Fremddaten 2006-21 (4 Stoerungen)."""
...
g = ("pf", 30, 1.2)
B = dict(r6.C510, minpayout=320.0)          # Gewinn >= 400 $ = 4 %
CFG = {
    "6.00 Sicher Ausz 4 %": (dict(B, db_on=0, r21_on=0, nz_on=0, gesamtbudget=0.9, idea_cap=0.9), F10, [0.75] * 10),
    "6.00 Ertrag Ausz 4 %": (dict(B, db_on=0, r21_risk=0.5, nz_risk=0.35, cool_n=3, gesamtbudget=0.9, idea_cap=0.9), F10, [0.75] * 10),
}
target = sys.argv[1]
if target == "ext":
...
for lbl, (kw, names, risks) in CFG.items():
    blks = ST.blocks(names, target, {n: g for n in names})
...
json.dump(res, open(f"x34_{target}.json", "w"), indent=1, default=float)
