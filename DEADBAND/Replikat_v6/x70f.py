"""Build 6.70: Zukunftstest 2026 mit eng11/x70.VAR (seit 6.60 verbraucht - nur berichtet, entscheidet nichts).
...
import x60, x70, x48, r6
import eng11 as E, evl6 as V, evl11 as V10

which = sys.argv[1].split("|") if len(sys.argv) > 1 else ["6.60"]
H = int(sys.argv[2]) if len(sys.argv) > 2 else 100
WARM = sys.argv[3] if len(sys.argv) > 3 else "2026-01-02"
fn = os.path.join(os.environ.get("X70_OUT", "ergebnisse"), "x70f.json")
res = json.load(open(fn)) if os.path.exists(fn) else {}
for lbl in which:
    key = f"{lbl} [H{H} ab {WARM}]"
    kw, gpx, frisk, rule, per, extra, extra_gp = x60.unpack(x70.VAR[lbl])
